...
1.1		- Caught conflict with options variable declaired within a theme functions file
		- Should solve other conflicts with wrongly coded variables from other plugins/themes
...
function wpcms_post_control_kill_iframes_init() {
	if ( strpos($_SERVER['REQUEST_URI'], 'wp-admin/post.php') === false )
		return;
	global $wpcmspc_options;
	if ( isset($wpcmspc_options['preview']) && $wpcmspc_options['preview'] == '0' )
		ob_start('wpcms_post_control_kill_preview');
	if ( isset($wpcmspc_options['uploading']) && $wpcmspc_options['uploading'] == '0' )
